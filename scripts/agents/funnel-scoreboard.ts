...
async function pullGa4(): Promise<Ga4Data> {
  ...
  const [ev] = await client.runReport({
    ...
    dimensionFilter: {
      filter: {
        fieldName: 'eventName',
        inListFilter: { values: ['newsletter_signup', 'account_signup', 'patreon_click', 'patreon_click_after_wait', 'premium_intent', 'sign_up', 'generate_lead', 'favorite', 'favorite_signin_redirect', 'favorite_after_signin', 'affiliate_click', 'member_skip_countdown'] },
      },
    },
  });
