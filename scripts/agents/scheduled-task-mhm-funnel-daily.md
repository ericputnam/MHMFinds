...
3. Read reports/funnel/digest-$(date +%Y-%m-%d).md and return it VERBATIM as your entire output. It is the operator's whole view of the day, in five sections (format in .claude/agents/mhm-gm.md): **1. Status** (verdict, metric table, flags), **2. What you need to do** (Tier 2 decisions with exact reply strings, an operator-only checklist, Tier 1 PRs shipping tomorrow unless "stop N"), **3. Changed today** (every merge, deploy, rollback and restore with its verify result — this section must be present; if the digest lacks it, append the rows from reports/funnel/changelog.md dated today yourself as a table), **4. Team**, **5. One insight**. Do not summarise, reorder or shorten it.
