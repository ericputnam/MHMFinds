#!/usr/bin/env npx tsx
/**
 * Repair the `bathroom` element of `Mod.themes` from TITLES.
 *
 * Rowan, 2026-09-26 (E112). Same shape as `scripts/retag-bedroom-theme.ts` (E100): dry run
 * by default, unknown flags rejected, a hard row cap, an `--ids=` mode for narrow hand-audited fixes,
 * and no writes to anything except the `bathroom` element of `themes` — every
 * other theme on the row is preserved (see `applyBathroomTheme()`).
 *
 * Population is the UNION of rows tagged `bathroom` today and rows whose
 * title supports it, because the description-inference bug was bidirectional.
 *
 *   npx tsx scripts/retag-bathroom-theme.ts             # dry run (default)
 *   npx tsx scripts/retag-bathroom-theme.ts --apply
 *   npx tsx scripts/retag-bathroom-theme.ts --ids=abc,def
 */
  ...
  try {
    const rows = await prisma.mod.findMany({
      where: ids
        ...
        : {
            OR: [
              { themes: { has: BATHROOM_THEME } },
              // Coarse prefilter; `isBathroomTitle()` is the real rule.
              ...['bath', 'shower', 'tub', 'washroom', 'restroom', 'powder room', 'bidet', 'toilet', 'towel'].map(
    ...
    const changes = rows
      .map((r) => ({ r, next: applyBathroomTheme(r.themes, r.title) }))
      .filter(({ r, next }) => next.includes(BATHROOM_THEME) !== r.themes.includes(BATHROOM_THEME));
    const added = changes.filter(({ next }) => next.includes(BATHROOM_THEME));
    const stripped = changes.filter(({ next }) => !next.includes(BATHROOM_THEME));
    const after = rows.filter((r) => isBathroomTitle(r.title));

    console.log(`population ${rows.length}; tagged before ${rows.filter((r) => r.themes.includes(BATHROOM_THEME)).length}; tagged after ${after.length}`);
    ...
    const backup = `reports/funnel/bathroom-theme-retag-${new Date().toISOString().slice(0, 10)}.csv`;
