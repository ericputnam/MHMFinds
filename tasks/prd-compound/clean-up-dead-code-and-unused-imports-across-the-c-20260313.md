...
**Date**: 2026-03-13
**Branch**: `compound/clean-up-dead-code-and-unused-imports-across-the-c-20260313`
**Prior PRDs**: `...-20260311.md`, `...-20260312.md` (same scope, refreshed with corrected findings)

## 1. Overview

**What**: Remove unused imports, dead code, commented-out code, and orphaned components across the codebase. Extract duplicated constants into a shared module.

**Why**: Unused imports inflate bundle size (especially lucide-react icons that aren't tree-shaken), create false leads during debugging, and add cognitive noise. The March 12 PRD incorrectly flagged `app/admin/mods/page.tsx` icons as unused — they are all used in JSX. This PRD corrects that and adds two newly found unused imports in `app/mods/[id]/page.tsx`.
...
| `app/mods/[id]/page.tsx:19` | `MessageSquare` | Remove from lucide-react import |
| `app/mods/[id]/page.tsx:21` | `Users` | Remove from lucide-react import |

**Correction from Mar 12 PRD**: `app/admin/mods/page.tsx` icons (`Edit`, `Download`, `ExternalLink`, `Check`) are **NOT unused** — all are referenced in JSX. Removed from scope.
...
| Files | Issue | Action |
|-------|-------|--------|
| `components/ModCard.tsx:11-42`, `components/ModDetailsModal.tsx:10-54` | Identical `FACET_COLORS` object duplicated (27 facet entries, ~42 lines each) | Extract to `lib/facetColors.ts`, import in both files |
...
- **`app/admin/mods/page.tsx` icons**: All 4 icons (`Edit`, `Download`, `ExternalLink`, `Check`) are used in JSX — incorrectly flagged in prior PRD.
...
| `app/mods/[id]/page.tsx` | Remove 2 unused icons from import (`MessageSquare`, `Users`) |
...
| `MessageSquare`/`Users` used in commented-out or conditional code | Low | Medium | Verified with line-level grep — only appear on import lines in `page.tsx` |
