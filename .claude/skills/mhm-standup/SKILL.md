---
name: mhm-standup
description: Run the MustHaveMods daily team pulse. Use when the user says "/mhm-standup", "daily standup", "morning pulse", or "how's the site doing today". Max, Tim, and Mark each return a one-line 🟢/🟡/🔴 status; anything red is escalated. Light and fast — most days it's all green.
...
Steps:
1. Spawn **Max, Tim, Mark in parallel** (Agent: `mhm-ad-revenue`, `mhm-growth`,
   `mhm-finance`). Brief each: "Daily pulse only. Return ONE line: 🟢/🟡/🔴 + the
...
2. Summarize the three pulses in 3 lines for the operator.
