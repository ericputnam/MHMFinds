...
- **"Decision documented" ≠ "capability built" ≠ "capability merged" ≠ "capability live"**: as of 2026-09-07 the newsletter SMTP work sits in all four states at once — the decision to use BigScoots SMTP over SendGrid is merged (`c022917`), the implementation is complete on `funnel/cass/smtp-transport` but not on main, and even once merged it is inert until the 5 `SMTP_*` vars exist in Vercel (0 of 5 today). Likewise the homepage SSR shell (`4fb1a66`, PR #48) is **not** on main — the homepage still ships ~20KB of client-only shell with 0 `<h1>` to crawlers. Do not attribute any GSC or revenue movement to either until they land and the env vars are set.
...
*Last compound review: 2026-09-07*
